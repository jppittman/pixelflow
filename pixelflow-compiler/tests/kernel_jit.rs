...
#[test]
fn verify_jit_macro_return_x() {
    let m = kernel_jit!(|| X);
    assert_eq!(eval1(&m, 42.0), 42.0);
}

#[test]
fn verify_jit_macro_add_xy() {
    let m = kernel_jit!(|| X + Y);
    assert_eq!(eval2(&m, 10.0, 32.0), 42.0);
}

#[test]
fn verify_jit_macro_complex_expr() {
    // (X + Y) * Z
...
#[test]
fn verify_jit_macro_subtraction() {
    let m = kernel_jit!(|| X - Y);
    assert_eq!(eval2(&m, 100.0, 58.0), 42.0);
}

#[test]
fn verify_jit_macro_division() {
    let m = kernel_jit!(|| X / Y);
    assert_eq!(eval2(&m, 84.0, 2.0), 42.0);
}

#[test]
fn verify_jit_macro_negation() {
    let m = kernel_jit!(|| -X);
    assert_eq!(eval1(&m, -42.0), 42.0);
}
...
#[test]
fn verify_jit_macro_abs() {
    let m = kernel_jit!(|| X.abs());
    assert_eq!(eval1(&m, -42.0), 42.0);
}

#[test]
fn verify_jit_macro_min_max() {
