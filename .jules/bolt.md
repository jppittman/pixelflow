...
## 2025-07-03 - Terminal Snapshot Row/Cell Item Vector Allocation Overheads
**Learning:** In the performance critical terminal snapshot renderer (`core-term/src/terminal_app.rs`), vectors used for holding positioned cell and row BSP items (`cell_items` and `row_items`) were being initialized on every rendering frame with `Vec::new()`, which involves dynamic reallocations as elements are added during rasterization iteration. Given the layout of grids, their capacities can often be known upfront using the snapshot dimensions (`rows`, `cols`), meaning initializing with `Vec::with_capacity()` bypasses significant heap allocation overhead. Also adding an early check if `rows == 0 || cols == 0` completely bypasses allocation and evaluation logic altogether for zero-sized contexts.
**Action:** Always pre-allocate vectors (`Vec::with_capacity()`) where exact upper bounds are known in loops, particularly in render loops. Additionally, short circuit and return early for zero inputs before initializing vectors.
