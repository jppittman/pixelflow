...
fn it_should_print_a_single_multibyte_unicode_character() {
    ...
    let glyph_1_wrapper = get_glyph_from_snapshot(&snapshot, 0, 0).expect("Should be present");
    let glyph_2_wrapper = get_glyph_from_snapshot(&snapshot, 0, 1).expect("Should be present");
...
fn it_should_print_multiple_multibyte_unicode_characters() {
    ...
    let char1_glyph1 = get_glyph_from_snapshot(&snapshot, 0, 0).expect("Should be present");
    let char1_glyph2 = get_glyph_from_snapshot(&snapshot, 0, 1).expect("Should be present");
    match char1_glyph1 {
    ...
    let char2_glyph1 = get_glyph_from_snapshot(&snapshot, 0, 2).expect("Should be present");
    let char2_glyph2 = get_glyph_from_snapshot(&snapshot, 0, 3).expect("Should be present");
    match char2_glyph1 {
...
fn it_should_handle_mixed_ascii_and_multibyte_unicode_characters() {
    ...
    let glyph_a = get_glyph_from_snapshot(&snapshot, 0, 0).expect("Should be present");
    match glyph_a {
        Glyph::Single(cell) => assert_eq!(cell.c, 'A'),
        _ => panic!("Expected Single 'A' at (0,0)"),
    }

    let glyph_uni_1 = get_glyph_from_snapshot(&snapshot, 0, 1).expect("Should be present");
    let glyph_uni_2 = get_glyph_from_snapshot(&snapshot, 0, 2).expect("Should be present");
    match glyph_uni_1 {
        Glyph::WidePrimary(cell) => assert_eq!(cell.c, '世'),
    ...
    let glyph_b = get_glyph_from_snapshot(&snapshot, 0, 3).expect("Should be present");
    match glyph_b {
...
fn it_should_not_print_second_half_of_wide_char_if_at_edge_and_no_wrap_mode_or_similar_logic() {
    ...
    let glyph_uni_1 = get_glyph_from_snapshot(&snapshot, 0, 0).expect("Should be present");
    match glyph_uni_1 {
        Glyph::WidePrimary(cell) => assert_eq!(cell.c, '世'),
        _ => panic!("Expected WidePrimary '世' at (0,0)"),
    }
    if snapshot.dimensions.0 > 1 {
        let glyph_uni_2 = get_glyph_from_snapshot(&snapshot, 0, 1).expect("Should be present");
        assert!(
            matches!(glyph_uni_2, Glyph::WideSpacer),
            "Expected WideSpacer at (0,1)"
        );
...
fn it_should_overwrite_first_half_of_wide_char_with_ascii() {
    ...
    let glyph_x = get_glyph_from_snapshot(&snapshot, 0, 0).expect("Should be present");
    match glyph_x {
        Glyph::Single(cell) => assert_eq!(cell.c, 'X'),
        _ => panic!("Expected Single 'X' at (0,0)"),
    }
    let glyph_after_x = get_glyph_from_snapshot(&snapshot, 0, 1).expect("Should be present");
    match glyph_after_x {
        Glyph::Single(cell) => {
            assert_eq!(
                cell.c, ' ',
...
fn it_should_print_ascii_over_wide_char_that_straddles_line_end_after_wrap() {
    ...
    let glyph_s4_0_0 = get_glyph_from_snapshot(&s4, 0, 0).expect("Should be present");
    let glyph_s4_0_1 = get_glyph_from_snapshot(&s4, 0, 1).expect("Should be present");
    let glyph_s4_1_0 = get_glyph_from_snapshot(&s4, 1, 0).expect("Should be present");
    let glyph_s4_1_1 = get_glyph_from_snapshot(&s4, 1, 1).expect("Should be present");
    ...
    let glyph_at_0_0 = get_glyph_from_snapshot(&s4, 0, 0).expect("Should be present");
    assert!(
    ...
    let glyph_at_0_1 = get_glyph_from_snapshot(&s4, 0, 1).expect("Should be present");
    match glyph_at_0_1 {
        Glyph::Single(cell) => assert_eq!(cell.c, 'Z'),
        _ => panic!("Expected Single at (0,1)"),
    }
...
fn it_should_show_and_hide_cursor_on_dectcem() {
    ...
    assert_eq!(
        snapshot_shown.cursor_state.expect("Should be present").shape,
        CursorShape::Block
    );
}
...
fn it_should_reset_all_attributes_on_sgr_0() {
    ...
    let snapshot = term.get_render_snapshot().expect("Snapshot was None");
    let glyph_a_wrapper = get_glyph_from_snapshot(&snapshot, 0, 0).expect("Should be present");
    let glyph_b_wrapper = get_glyph_from_snapshot(&snapshot, 0, 1).expect("Should be present");
...
fn it_should_set_bold_on_sgr_1_and_reset_on_sgr_22() {
    ...
    let snapshot = term.get_render_snapshot().expect("Snapshot was None");
    let attr_a = match get_glyph_from_snapshot(&snapshot, 0, 0).expect("Should be present") {
        Glyph::Single(c) => c.attr,
        _ => panic!("Expected Single"),
    };
    let attr_b = match get_glyph_from_snapshot(&snapshot, 0, 1).expect("Should be present") {
        Glyph::Single(c) => c.attr,
        _ => panic!("Expected Single"),
    };
...
fn it_should_set_basic_ansi_foreground_colors_sgr_30_37() {
    ...
    let snapshot = term.get_render_snapshot().expect("Snapshot was None");
    for (i, &color_name) in colors.iter().enumerate() {
        let glyph_wrapper = get_glyph_from_snapshot(&snapshot, 0, i).expect("Should be present");
        match glyph_wrapper {
            Glyph::Single(cell) => {
