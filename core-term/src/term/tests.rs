...
fn csi_sgr_fg_color() {
    ...
    let glyph_b_wrapper = get_glyph_from_snapshot(&snapshot_b, 0, 1).expect("Glyph should exist at snapshot");
...
fn snapshot_with_selection() {
    ...
    let sel_range = snapshot_with_selection.selection.range.expect("Value should be present");
...
fn ps1_multiline_with_sgr_at_bottom_scrolls() {
    ...
    let glyph_p_wrapper = get_glyph_from_snapshot(&snapshot, 0, 0).expect("Glyph should exist at snapshot");
    let glyph_1_wrapper = get_glyph_from_snapshot(&snapshot, 0, 1).expect("Glyph should exist at snapshot");
    let glyph_dollar_wrapper = get_glyph_from_snapshot(&snapshot, 1, 0).expect("Glyph should exist at snapshot");
    let glyph_space_after_dollar_wrapper = get_glyph_from_snapshot(&snapshot, 1, 1).expect("Glyph should exist at snapshot");
    let glyph_final_cursor_cell_wrapper = get_glyph_from_snapshot(&snapshot, 1, 2).expect("Glyph should exist at snapshot");
