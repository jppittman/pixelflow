...
#[test]
fn simple_char_input_should_work() {
...
#[test]
fn carriage_return_input_should_work() {
    let mut term = create_test_emulator(10, 1);
    term.interpret_input(EmulatorInput::Ansi(AnsiCommand::Print('A')));
    term.interpret_input(EmulatorInput::Ansi(AnsiCommand::Print('B')));
...
#[test]
fn csi_cursor_forward_cuf_should_work() {
...
#[test]
fn csi_ed_clear_below_csi_j_should_work() {
...
#[test]
fn csi_sgr_fg_color_should_work() {
    ...
    let snapshot = term.get_render_snapshot().expect("Snapshot was None");
    let glyph_a_wrapper = get_glyph_from_snapshot(&snapshot, 0, 0).expect("Value should be present and valid");
    ...
    let glyph_b_wrapper = get_glyph_from_snapshot(&snapshot_b, 0, 1).expect("Value should be present and valid");
...
#[test]
fn mouse_press_starts_selection_should_work() {
...
#[test]
fn mouse_drag_updates_selection_should_work() {
    let mut emu = create_test_emulator(10, 5);
...
#[test]
fn mouse_release_ends_selection_activity_should_work() {
    let mut emu = create_test_emulator(10, 5);
    send_mouse_input(&mut emu, start_selection_at(1, 1), MouseButton::Left);
...
// --- Copy Action Tests ---
#[test]
fn initiate_copy_no_selection_should_work() {
    let mut emu = create_test_emulator(10, 5);
...
#[test]
fn initiate_copy_with_selection_should_work() {
...
#[test]
fn initiate_copy_block_selection_should_work() {
...
#[test]
fn new_mouse_press_clears_old_selection_should_work() {
    let mut emu = create_test_emulator(10, 5);
...
#[test]
fn selection_coordinates_adjust_on_scroll_should_work() {
...
#[test]
fn selection_on_alt_screen_then_exit_should_work() {
    let mut emu = create_test_emulator(10, 3);
    fill_emulator_screen(
        &mut emu,
...
#[test]
fn resize_smaller_content_truncation_should_work() {
    let mut term = create_test_emulator(5, 2);
...
#[test]
fn osc_set_window_title_should_work() {
    let mut term = create_test_emulator(10, 1);
...
#[test]
fn key_event_printable_char_should_work() {
    let mut term = create_test_emulator(5, 1);
...
#[test]
fn key_event_arrow_up_should_work() {
    let mut term = create_test_emulator(5, 1);
    let key_input = UserInputAction::KeyInput {
        symbol: KeySymbol::Up,
...
#[test]
fn snapshot_with_selection_should_work() {
    let num_cols = 10;
    let num_rows = 2;
    ...
    let sel_range = snapshot_with_selection.selection.range.expect("Value should be present and valid");
...
#[test]
fn mode_show_cursor_dectcem_should_work() {
    let mut term = create_test_emulator(5, 1);
    ...
    let initial_shape = snap_default.cursor_state.as_ref().expect("Value should be present and valid").shape;
    ...
    assert_eq!(
        snap_shown.cursor_state.as_ref().expect("Value should be present and valid").shape,
        initial_shape,
...
#[test]
fn ps1_multiline_prompt_at_bottom_causes_scroll_should_work() {
...
#[test]
fn ps1_multiline_prompt_ends_on_last_line_no_scroll_by_prompt_should_work() {
    let mut term = create_test_emulator(5, 3);
...
#[test]
fn ps1_multiline_prompt_last_line_fills_screen_then_input_should_work() {
    let mut term = create_test_emulator(3, 2);
...
#[test]
fn ps1_prompt_causes_multiple_scrolls_should_work() {
    let mut term = create_test_emulator(3, 2);
...
#[test]
fn ps1_prompt_with_internal_wrapping_and_scrolling_should_work() {
    let mut term = create_test_emulator(3, 2);
    for _ in 0..3 {
        term.interpret_input(EmulatorInput::Ansi(AnsiCommand::Print('A')));
    }
...
#[test]
fn ps1_multiline_exact_fill_then_scroll_on_final_lf_should_work() {
    let mut term = create_test_emulator(3, 2);
...
#[test]
fn ps1_multiline_with_sgr_at_bottom_scrolls_should_work() {
    let mut term = create_test_emulator(5, 2);
    ...
    let glyph_p_wrapper = get_glyph_from_snapshot(&snapshot, 0, 0).expect("Value should be present and valid");
    let glyph_1_wrapper = get_glyph_from_snapshot(&snapshot, 0, 1).expect("Value should be present and valid");
    let glyph_dollar_wrapper = get_glyph_from_snapshot(&snapshot, 1, 0).expect("Value should be present and valid");
    let glyph_space_after_dollar_wrapper = get_glyph_from_snapshot(&snapshot, 1, 1).expect("Value should be present and valid");
    let glyph_final_cursor_cell_wrapper = get_glyph_from_snapshot(&snapshot, 1, 2).expect("Value should be present and valid");
...
#[test]
fn lf_at_bottom_of_partial_scrolling_region_no_origin_mode_should_work() {
    let cols = 10;
    let rows = 5;
...
#[test]
fn primary_device_attributes_response_should_work() {
...
mod selection_logic_tests {
    ...
    #[test]
    fn start_selection_should_work() {
        let mut emu = create_test_emulator(10, 5);
    ...
    #[test]
    fn extend_selection_active_and_inactive_should_work() {
        let mut emu = create_test_emulator(10, 5);
    ...
    #[test]
    fn apply_selection_clear_click_and_drag_should_work() {
        let mut emu = create_test_emulator(10, 5);
    ...
    #[test]
    fn clear_selection_should_work() {
        let mut emu = create_test_emulator(10, 5);
...
mod get_selected_text_tests {
    ...
    #[test]
    fn get_selected_text_no_selection_should_work() {
        let emu = create_test_emulator(10, 5);
        assert_eq!(emu.get_selected_text(), None);
    }

    #[test]
    fn get_selected_text_single_line_should_work() {
        let mut emu = create_test_emulator(10, 5);
    ...
    #[test]
    fn get_selected_text_single_line_trailing_spaces_in_selection_should_work() {
    ...
    #[test]
    fn get_selected_text_multi_line_should_work() {
        let mut emu = create_test_emulator(10, 5);
        fill_emulator_screen(
            &mut emu,
            vec![
    ...
    #[test]
    fn get_selected_text_multi_line_full_lines_should_work() {
        let mut emu = create_test_emulator(10, 3);
        fill_emulator_screen(
            &mut emu,
            vec![
    ...
    #[test]
    fn get_selected_text_line_boundaries_should_work() {
        let mut emu = create_test_emulator(10, 2);
        fill_emulator_screen(
            &mut emu,
    ...
    #[test]
    fn get_selected_text_empty_cells_within_grid_should_work() {
    ...
    #[test]
    fn get_selected_text_selection_beyond_line_length_should_work() {
        let mut emu = create_test_emulator(10, 1);
    ...
    #[test]
    fn get_selected_text_reversed_points_should_work() {
        let mut emu = create_test_emulator(10, 5);
        fill_emulator_screen(&mut emu, vec!["Hello World".to_string()]);
...
mod paste_text_tests {
    ...
    #[test]
    fn paste_text_bracketed_off_simple_should_work() {
    ...
    #[test]
    fn paste_text_bracketed_off_with_newline_should_work() {
    ...
    #[test]
    fn paste_text_bracketed_off_causes_wrap_should_work() {
    ...
    #[test]
    fn paste_text_bracketed_on_logs_warning_processes_chars_should_work() {
        let mut emu = create_test_emulator(20, 1);
    ...
    #[test]
    fn ansi_resize_sets_terminal_dimensions_should_work() {
    ...
    #[test]
    fn ansi_resize_updates_snapshot_dimensions_should_work() {
        let mut emu = create_test_emulator(80, 24);
    ...
    #[test]
    fn ansi_resize_with_zero_rows_ignored_should_work() {
        let mut emu = create_test_emulator(80, 24);
    ...
    #[test]
    fn ansi_resize_with_zero_cols_ignored_should_work() {
        let mut emu = create_test_emulator(80, 24);
    ...
    #[test]
    fn ansi_resize_with_missing_params_ignored_should_work() {
        let mut emu = create_test_emulator(80, 24);
    ...
    #[test]
    fn window_manipulation_report_size_should_work() {
        let mut emu = create_test_emulator(80, 24);
