...
fn test_csi_sgr_fg_color() {
    ...
    let glyph_b_wrapper = get_glyph_from_snapshot(&snapshot_b, 0, 1).expect("Expected value to be present");
...
fn test_snapshot_with_selection() {
    ...
    let sel_range = snapshot_with_selection.selection.range.expect("Expected value to be present");
...
fn test_mode_show_cursor_dectcem() {
    ...
    let initial_shape = snap_default.cursor_state.as_ref().expect("Expected value to be present").shape;
    ...
    assert_eq!(
        snap_shown.cursor_state.as_ref().expect("Expected value to be present").shape,
        initial_shape,
...
fn test_ps1_multiline_with_sgr_at_bottom_scrolls() {
    ...
    let glyph_p_wrapper = get_glyph_from_snapshot(&snapshot, 0, 0).expect("Expected value to be present");
    let glyph_1_wrapper = get_glyph_from_snapshot(&snapshot, 0, 1).expect("Expected value to be present");
    let glyph_dollar_wrapper = get_glyph_from_snapshot(&snapshot, 1, 0).expect("Expected value to be present");
    let glyph_space_after_dollar_wrapper = get_glyph_from_snapshot(&snapshot, 1, 1).expect("Expected value to be present");
    let glyph_final_cursor_cell_wrapper = get_glyph_from_snapshot(&snapshot, 1, 2).expect("Expected value to be present");
