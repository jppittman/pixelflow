...
fn cuj_pty01_single_byte_batch_delivery() {
    ...
    parser_tx.send(Message::Data(test_data.clone())).expect("test failure");
...
fn cuj_pty01_large_batch_handling() {
    ...
    parser_tx.send(Message::Data(large_data)).expect("test failure");
...
fn cuj_pty01_channel_closure_propagation() {
    ...
    // Send some data
    parser_tx.send(Message::Data(b"test".to_vec())).expect("test failure");
...
fn cuj_pty02_command_batch_delivery() {
    ...
    cmd_tx.send(commands.clone()).expect("test failure");

    // Then: App should receive the exact batch
    let received = cmd_rx.recv_timeout(Duration::from_millis(100)).expect("test failure");
...
fn cuj_pty02_empty_input_no_output() {
    ...
    parser_tx.send(Message::Data(vec![])).expect("test failure");
...
fn cuj_pty02_mixed_content_batch() {
    ...
    parser_tx.send(Message::Data(mixed_data)).expect("test failure");
...
fn cuj_pty03_write_command_delivery() {
    ...
    write_tx.send(output.clone()).expect("test failure");

    // Then: Write thread receives the bytes
    let received = write_rx.recv_timeout(Duration::from_millis(100)).expect("test failure");
...
fn cuj_pty03_write_ordering_preserved() {
    ...
    for i in 0..10 {
        write_tx.send(vec![b'0' + i]).expect("test failure");
    }
...
fn cuj_pty04_sender_drop_terminates_receiver() {
    ...
    // Send some data
    tx.send(Message::Data(42)).expect("test failure");
    thread::sleep(Duration::from_millis(20));
...
impl Actor<MockEngineData, MockEngineControl, MockEngineManagement> for MockTerminalAppActor {
    fn handle_data(&mut self, data: MockEngineData) -> HandlerResult {
        self.data_events.lock().expect("test failure").push(data);
        Ok(())
    }

    fn handle_control(&mut self, ctrl: MockEngineControl) -> HandlerResult {
        self.control_events.lock().expect("test failure").push(ctrl);
        Ok(())
    }

    fn handle_management(&mut self, mgmt: MockEngineManagement) -> HandlerResult {
        self.management_events.lock().expect("test failure").push(mgmt);
        Ok(())
    }
...
fn cuj_eng01_resize_message_delivery() {
    ...
    let events = control_events.lock().expect("test failure");
...
fn cuj_eng04_keydown_delivery() {
    ...
    let events = management_events.lock().expect("test failure");
...
fn cuj_eng09_frame_request_delivery() {
    ...
    let events = data_events.lock().expect("test failure");
    assert_eq!(events.len(), 1);
...
fn cuj_priority_control_before_management_before_data() {
    ...
    impl Actor<String, String, String> for OrderRecordingActor {
        fn handle_data(&mut self, msg: String) -> HandlerResult {
            self.order.lock().expect("test failure").push(format!("D:{}", msg));
            Ok(())
        }
        fn handle_control(&mut self, msg: String) -> HandlerResult {
            self.order.lock().expect("test failure").push(format!("C:{}", msg));
            Ok(())
        }
        fn handle_management(&mut self, msg: String) -> HandlerResult {
            self.order.lock().expect("test failure").push(format!("M:{}", msg));
            Ok(())
        }
        fn park(&mut self, _: SystemStatus) -> Result<ActorStatus, HandlerError> {
            Ok(ActorStatus::Idle)
    ...
    tx.send(Message::Data("data1".to_string())).expect("test failure");
    tx.send(Message::Data("data2".to_string())).expect("test failure");
    tx.send(Message::Management("mgmt1".to_string())).expect("test failure");
    tx.send(Message::Control("ctrl1".to_string())).expect("test failure");
    ...
    let order = message_order.lock().expect("test failure");
    ...
    assert!(
        ctrl_idx.expect("test failure") < first_data_idx.expect("test failure"),
...
fn cuj_concurrent_senders_all_messages_delivered() {
    ...
    for (sender_id, tx) in sender_handles_vec.drain(..).enumerate() {
        let handle = thread::spawn(move || {
            for msg_id in 0..MESSAGES_PER_SENDER {
                tx.send(Message::Data(sender_id * 1000 + msg_id)).expect("test failure");
            }
        });
    ...
    thread::sleep(Duration::from_millis(100));
    receiver_handle.join().expect("test failure");
