...
| [a glyph is a formula](plans/2026-09-23-a-glyph-is-a-formula.md) | nothing — denotation only (2026-09-23) | **F1** — a range is a value: sibling reductions over one range are one loop scope; **F2** — an arm may own a root when it owns every scope that reads it, so a glyph's box skips its folds; **F3** — the piece tree (boxes, leaves) as bound data, one program per font | Baseline in [results/2026-09-23-freetype-comparison.md](results/2026-09-23-freetype-comparison.md): `8`@32 collapse 153 µs on AVX-512 vs FreeType 5–8 µs per glyph; a 50-glyph run 230× FreeType because the per-glyph box selects own none of their folds (guard telemetry). The H6 stack removed the instruction overhead; what is left is the algorithm, and the plan puts it in the compiler rather than the font code. |
