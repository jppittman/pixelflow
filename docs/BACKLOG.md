...
| **R1–R3** | **A surviving `Reduce` is a loop.** The emitter is handed 34,993 straight-line instructions for one glyph because `ExpandReduce` unrolls unconditionally and then gather/transcendental expansion multiplies each copy ~4×; `emit` is ~O(n^1.6) in that. A loop makes the program the *body* — ~1,030 entries — and the accumulator lives in a **slot**, exactly as `emit_collapse_loop` keeps its coordinate, so nothing is live across the back edge and `LinearScan` needs no change. **This is the hump's root cause, not H1.** | [a-surviving-reduce-is-a-loop](plans/2026-09-10-a-surviving-reduce-is-a-loop.md) |
