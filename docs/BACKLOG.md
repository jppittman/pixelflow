...
| an `If`'s arms — **blocks** | flattening the DAG to a linear schedule | `cluster_if_arms`, which was a permutation *search* and 73% of a glyph bake (**H5**, now one pass) |
...
(`cluster_if_arms`, 73% of a bake). H1 alone — one program per font
...
`cluster_if_arms` finding) are in
...
| **D2** | Lowering 1 — emit the split: `If` over the derived range, root specialized at `m ≡ false` over the complement. | *ibid.* |
...
| **D5** | Lowering 2 on the general predicate — the superseded demand plan's §1–§2, as the third case rather than the whole subject. **Does not delete `cluster_if_arms`**: `emit/demand.rs` already computes the predicate and disproved that plan's scheduling claim (see the correction in [results](results/2026-09-10-glyph-bake-hump.md)). What it buys is *more* exclusivity than `guards` finds — the per-`If` `demand_exclusive` vs `exclusive` gap under `PIXELFLOW_GUARD_TELEMETRY` — and `demand.rs` says outright that this gap, not the scheduling claim, is what C1b should be justified by. Read that measurement before starting. | *ibid.* |
...
| **D7** | **`Guard` in the graph, and the sink rule** (C2b) — *guarding becomes the e-graph's decision instead of codegen's.* Today it is neither: `analyze_if_guards` runs on the linearized schedule, after extraction, and the emitter places branches from it. Worse, the graph pulls the other way — `IfHoistUnary` rewrites `If(m, f(a), f(b)) → f(If(m, a, b))`, hoisting shared work *out* of arms, which is right for op count and exactly wrong for guarding when `f` is expensive and `m` is coherent, and **no term in the extraction cost opposes it**. Denote `If(m, a, b)` as `Guard(m, a) ⊕ Guard(¬m, b)`, add the inverse sink rule, and let the cost decide. Needs D6 for the term. Composes with N1: `Guard(m, Ref(a))` carries its arm as a unit, which is what makes H5's partition *unsayable* rather than merely cheap. | [demand-is-a-dag-property](plans/2026-09-07-demand-is-a-dag-property.md) §4 |
...
| **C2** | **The `'8'` waist bug is open on `main`.** Five fixes tried and refuted; `freetype_oracle.rs` pins it rather than fixing it. The general demand predicate (D5), not a sixth per-`If` patch, is the intended next attempt. | [demand-is-a-dag-property](plans/2026-09-07-demand-is-a-dag-property.md) §9 |
...
| **C7** | **The ISA is decided at startup.** ~~The tier was `cfg(target_feature)`, so a plain build ran 128-bit SSE2 kernels on every x86-64 host and each wider tier was a separate workspace build only `xtask isa-matrix` made.~~ **Landed**: `pixelflow_codegen::isa::detect` reads CPUID once per process — AVX-512 where the host has it, AVX2+FMA as the floor, NEON on aarch64 — and `PIXELFLOW_ISA` overrides downward for diagnosis and CI; `pixelflow-core` lost `Field`/`PARALLELISM`/its `backend` module; `isa-matrix` is one build and one clippy, then the tests once per tier. ~~**Open**: the follow-up deletes the SSE2 driver now that nothing selects it.~~ **Done** — the SSE2 driver (`X86Backend`, `SSE2_FILE`, ~510 lines), the 128-bit encoders only it called (~900 lines; `x86_64.rs` 2,935 → 1,411, the leaf-encoder module), `resolve_operands`' two-operand invariant and the SSE arms of the byte tests are gone; the AVX2 tier gathers with `vgatherdps` (two temps, no GPR) instead of two scalar-insert halves borrowed from SSE2. Glyph `8`@32 on AVX2 is unchanged at 4,360 B / 629 instructions: its table reads are all `Broadcast`s, so the gather change does not reach it. **Still open**: the `ISA matrix (SSE2/AVX2/AVX-512)` check name is renamed once branch protection's required-checks list is. | [the-isa-is-decided-at-startup](plans/2026-09-22-the-isa-is-decided-at-startup.md) §7 |
