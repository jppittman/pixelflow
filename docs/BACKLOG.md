...
| **C7** | **The ISA is decided at startup.** ~~The tier was `cfg(target_feature)`, so a plain build ran 128-bit SSE2 kernels on every x86-64 host and each wider tier was a separate workspace build only `xtask isa-matrix` made.~~ **Landed**: `pixelflow_codegen::isa::detect` reads CPUID once per process — AVX-512 where the host has it, AVX2+FMA as the floor, NEON on aarch64 — and `PIXELFLOW_ISA` overrides downward for diagnosis and CI; `pixelflow-core` lost `Field`/`PARALLELISM`/its `backend` module; `isa-matrix` is one build and one clippy, then the tests once per tier. **Open**: the follow-up deletes the SSE2 driver (`x86_64::driver::X86Backend`, its `SSE2_FILE`, `emit_gather_scalar` and the byte-pin tests on those encoders) now that nothing selects it, and the `ISA matrix (SSE2/AVX2/AVX-512)` check name is renamed once branch protection's required-checks list is. | [the-isa-is-decided-at-startup](plans/2026-09-22-the-isa-is-decided-at-startup.md) |
