...
| **H4** | **Ask B — hoist binder-only work out of the pixel loop.** ~~On the hump.~~ **Demoted by H2**: it optimizes *collapse*, which is 0.2% of a bake, and a glyph bakes once into the atlas and is a gather forever after. Still real for per-frame kernels that are not atlas-cached; not the terminal's startup problem. **Do not patch `contains_gather`** (N1) and do not write a new hoist (N5). **Subsumed by H6** §3.3: the same hoist, seen from the emitter's side. | [a-glyph-is-a-circle](plans/2026-09-09-a-glyph-is-a-circle.md) §B |
| **H6** | **A uniform read is one load.** Disassembled, `8`@32 on the AVX-512 tier is 48,452 instructions of which the Loop–Blinn arithmetic is ~3,000: the rest is 1,610 table reads, each a per-lane gather plus its address arithmetic and constant remats, unrolled 34 times. Three facts with three owners and **nothing in the language**: the nested winding fold as a loop (the surviving-reduce plan's remainder — the compile-time story); a `RawGather` whose index is X-invariant emitted as `vbroadcastss` like a `Uniform` load (the emitter, keyed on variance — the runtime story); and the stale gather exclusion in `plan_collapse_hoist` deleted so in-range table reads hoist. Records the refused alternatives (a post-e-graph fold pass, a `Row` node) and why. | [a-uniform-read-is-one-load](plans/2026-09-16-a-uniform-read-is-one-load.md) |
