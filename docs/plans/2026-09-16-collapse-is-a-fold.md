...
- **Status**: `In progress` — denotation settled 2026-09-17; implementation in
  the order of §5, one PR per row. Steps 1–6 and 5½ landed.
...
- **`collapse(extent)`**: `f ↦ fold_j fold_i fold_{l∈[0,1)} Write(j, i, l, f.at(x0 + i + l, y0 + j))`.
  Target-agnostic; `L` does not appear. The lane fold is *degenerate* — one
  trip — so the `Write` carries all three binders from the start and `pack`
  changes ranges only. `x0`, `y0` are two uniforms the caller declares
  (`passes::lattice::Domain`); `(x0 + i)` is its own node so the
  lane-uniform half of the address hoists to the batch scope.
- **`pack(L)`**: strip-mine `fold_i` into `fold_k fold_l` with `k` stepping by
  `L` (`Fold::strided`, the field HalveFold already uses) and `i := k + l`;
  when `r > 0`, sequence the remainder fold after it. Read monadically — a
  `SEQ` fold is `for_`, `Seq` is `>>` — it is the chunking law
  `for_ [0,w) f = for_ [0,w−r) step L (λk. for_ [0,L) (λl. f(k+l))) >> for_ [w−r] (λk. for_ [0,r) (λl. f(k+l)))`:
  the two inner folds' ranges move and nothing else does, so the `Write` is
  one shared node under both arms. `L` is the pass's one parameter, handed in
  by `pixelflow-codegen`; `pixelflow-ir` never names a width. HalveFold is
  this transform with `L = 2` and the inner fold unrolled; unifying them is a
  later subtraction, not this plan's.
...
| 4 | ~~**The two passes.** `collapse(extent)` and `pack(L)` in `legalize`, on an arena the emitter does not yet accept.~~ **Done.** `passes::lattice::{collapse, pack}` and `Fold::strided`; standalone until step 5 wires them into `legalize` (the emitter refuses a `Write` until then, so wiring now would break every collapse). `collapse` refuses a reachable `Ref`, `Guard` or `Dwrt` by name: substitution cannot reach through a name, and a derivative must be taken before its variable is replaced. | arena-level tests: the exact shape; the slots a kernel's own fold leaves free (no capture); every binder's bit, the root `CONST`; a `Gather` addressed by `Y` alone lacks the col and lane bits (the bit step 6 reads); `pack` for `r = 0`, `r > 0` and `w < L`, the `Write` one shared node; a foreign root refused | — |
| 5 | ~~**The emitter executes the lane fold and the `Write`; the scaffold is deleted; the ABI is `fn(ctx, out, pitch)`; `pixelflow-core` collapses in one call.**~~ **Done.** `legalize` takes a `Collapse { domain, lanes }` and runs the two passes between `lower_dwrt` and `expand_gather`; `arena_to_schedule` inlines each lane fold into its parent's schedule as a `Write { row, col, lane, lanes, value }` def (the fold's binder becomes the `Lanes` iota leaf, its trip count the store's width), so the two lane folds sharing one `Write` node are two defs of different widths reading one value. The lattice's row and column folds are folds like any other: `extract_folds` carves them, and what a region used to do — compute a value once and park it for the scopes inside — is `place_roots`, at every fold boundary at once, with the storing scope binding the lane. The origin is two process-global uniforms (`emit::origin()`) read from a context entry of their own, after the link's block, so a call copies nothing. Every backend gained `emit_write` (the address is `out + 4·(row·pitch + col)` from the binders — `cvttss2si`/`imul`/`add`/`lea`, or `fcvtzs`/`madd`/`add lsl #2` — and a remainder is `movss`+`psrldq`, `vextractps`, a `kmovw`-masked `vmovups`, or `st1 {v.s}[k]` per lane) and the iota (`movabs`+`movq`+`movlhps`, `vpmovzxbd`, `vpinsrq`+`vpmovzxbd`, a pool entry). Deleted: the scaffold and its mod, `Counter`/`OutStep`/`CollapseBody`/`Level`, `emit_collapse_loop`/`emit_nest`, `HoistCtx`, `partition_by_scope`, `Scope::Region`, `RegisterFile::inputs` (xmm0-3/v0-v3 are the allocator's now), the x86 red-zone spill mode, `Point4`/`TileSlice`/`call_collapse`, core's `BandPlan`/`RowTail`/`Destination`/`BATCH_LANES`/`Field::sequential` and both `collapse_subrect`s. A `Manifold` keeps a table of the shapes it has been collapsed at and compiles a new one on first use; a frame's stripes are two shapes. | `collapse_abi_smoke` and `collapse_paths` (a uniform, a gather, a nested reduce, a guarded select, a width below a batch — each with a remainder, each against scalar `f32`) on SSE2, AVX2 and AVX-512 on the same host; every graphics golden bit-exact; `bind_allocates_nothing`; `a_band_of_a_new_shape_compiles_once_and_is_reused`; `traffic` attributes every byte to one scope and counts one write per batch per row | the lattice's loops are scopes with `trips`, so `dynamic_memory_ops` needs no arguments; the SIMD width appears in no line of `pixelflow-core` |
| 5½ | ~~**Constants are values.**~~ **Done.** Three hand rules decided a constant's register, each written to hide the thrash the one before it caused: the eviction rank put a constant a tier below every other value regardless of when it is next read, the kept-reload step refused to bring one back because that rank would take it away again, and `stays_put` kept one out of every park because it was "rebuilt in one instruction" — which is one instruction for an FMOV-encodable immediate on aarch64 and two on every x86 tier (a `jmp` over sixteen bytes of inline data and a load; a red-zone store and a broadcast from it, a store-forward on the critical path). All three fold into one statement in the allocator's state: `in_slot` is true at a constant's birth. The rank's three tiers become `store: bool`; a constant is evicted when it is the farthest read among slot-valid values, and kept on a re-read like any value out of a register. A root's hand-off is recorded as a read at its own definition, so a constant parked for the scopes inside is written into a register first. `place_roots` parks a constant a fold reads like any root, priced by the carry plan and carried in a register when one is free; the lane binder is no longer counted as bound by the storing scope there, since the lanes are the same vector every batch — a value varying by lane alone is invariant over the call and belongs to the body (counting it bound left the storing scope holding the iota as a def and a live-in at once). Every x86 tier reads constants from one four-byte-per-entry pool after `ret`, anchored in `r8` by one `lea` (`vbroadcastss [r8 + disp32]`, one instruction), the design aarch64 had in `X17`; `CONST_POOL`/`CONST_POOL_ALIGN` are every backend's, and the two inline idioms are deleted. | allocator unit tests: distance decides between two constants, a constant is re-kept on a second read, a constant root materializes under pressure while an unread one still emits nothing, a constant a fold reads is carried across it; `a_leaf_feeding_both_scopes_is_parked_by_the_outer_one`; the pooled `emit_const`/`emit_unary` runtime tests on AVX2 and AVX-512; every graphics golden bit-exact; `xtask isa-matrix --clippy --smoke`. `constant_traffic` (pixelflow-graphics example) is the instrument. | This host's 128-bit tier. Glyph `8`@32: 10,005 → **7,288 B**; its two per-pixel folds 1,886 + 3,628 → **1,477 + 2,905 B**, 5 → **0** remats, 67 + 143 → 49 + 116 scheduled ops per trip. Cell grid, red channel at 960×8: 3,433 → **2,392 B**, its column loop 2,140 → **1,438 B** per batch with 24 → **0** remats; the packed four-channel frame program 5,773 → **3,672 B**. Counted memory ops rise (36,618 → 55,869 per stripe call): a remat was an uncounted load from the instruction stream, and a parked constant the sixteen-register pool cannot carry is now a counted frame load, one instruction where the remat was two. |
| 6 | ~~**Broadcast load** for a `Gather` whose address lacks the lane bit, split at `arena_to_schedule`.~~ **Done.** `arena_to_schedule` reads the index's variance (`compute_arena_variance`, since the schedule's own is computed after it is built) and emits `ScheduledOp::Broadcast` where the lane binder's bit is absent, `Gather` where it is present; a schedule with no lane fold keeps every read a gather. Placement, operands and the guard cost treat it as the gather it was; only the encoding changes: `cvttss2si` of lane 0, the buffer base from the context, and `vbroadcastss [base + idx*4]` on every x86 tier (a new SIB-scaled memory form, `scaled4_operand_into`, shared by the three prefixes), `fcvtzs x, s` + `ldr s, [base, w, uxtw #2]` + `dup` on NEON. No vector temps; two GPRs. | `broadcast::the_lane_bit_decides_broadcast_or_gather` (the split, both ways, and the no-lane-fold case); every backend's bytes pinned, the x86 ones checked with `objdump`; `collapse_paths`: `table[y] + x` and a fold reading `table[k]` by its own binder, each at a width with a remainder against scalar `f32`; goldens; `avx512_evex_proof` | `8`@32 on this host's tier: `vpextrd` 140 → **0**, `vinsertps` 105 → **0**, `vmovss` 140 → **0**, `vcvttps2dq` 35 → **0**; `vbroadcastss` 10 → 45 and `cvttss2si` 2 → 37, one each per read; 10,005 → **7,912 B**, 2,444 → **2,090** instructions. ~~What remains per read is its `mov base, [ctx + slot*8]` — 35 loads of one call-invariant pointer per batch, because no GPR is allocated across instructions yet~~ — closed by [a-pointer-is-a-value](2026-09-22-a-pointer-is-a-value.md): the base is a `Context` def the allocator places, 45 loads per batch → 3 per call |
...
- ~~**Where the write's address is computed.** From the row and batch counters
  ...
  each scope's close. The emitter's choice; whichever is fewer lines.~~ From
  the binders, wherever their folds keep them: the address is four GPR ops
  per store, and an induction pointer would have been one more root for the
  allocator to place and one more thing a scope's close had to know.
- ~~**`Fold`'s `u16` ends.** A frame width fits; the control-plane rule says
  widen anyway, and step 3 is where.~~ Widened to `u32` in step 3.
- ~~**The iota is rebuilt per batch.** `Lanes` is a leaf `place_roots` never
  parks, and its variance names the lane binder, which only the storing
  scope binds — so on the x86 tiers it is four to six instructions and a
  GPR per batch (`movabs`/`movq`/`vpmovzxbd`), where NEON reads one pool
  entry.~~ Closed by step 5½, the second way: `Lanes` left `stays_put`, and
  `place_roots` no longer counts the lane binder as bound by the storing
  scope, so a value that varies by lane alone is a root of the body — the
  iota is built once per call and read from its park or a carried register.
- ~~**A constant is placed in every scope that can see it, read or not.** A
  ...
  rebuilds them where they are read.~~ Closed by step 5½, the other way
  round from the fix proposed here: a constant is a root of the outermost
  scope that can compute it and a placeholder in every scope inside, so the
  copies are gone and the dead definitions with them.
