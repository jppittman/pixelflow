...
**Phase 3 pre-flight — oracle headroom at corpus scale.** ✅ **MEASURED 2026-08-30, revised
2026-09-01** (scoping only — no Guide built). The analogue of Phase 2's static/noswap=0.54 gate:
over 800 stride-sampled expressions from a regenerated `corpus_train.bin`/`corpus_dev.bin`,
median per-expression load-bearing ratio is 38% under the hindsight labeler (implying ~2.6x
oracle savings) and 2.9% under a strict lower bound that credits only applications whose output
node is literally on the extracted derivation path (implying ~34x). Headroom clears the bar to
run Phase 3. The load-bearing new finding, not previously visible from the 5-toy-kernel rule
report: **the labeler and strict bounds correlate only moderately overall** (Spearman ρ ≈ 0.35,
n=55 rule instances that fired — a 2026-09-01 PR-review-driven correction; an earlier draft
misreported this as ρ≈0.02 from an uncomputed, non-reproducible number) **and split cleanly by
rule class** — the core structural/congruence rules (commutative, fma-fusion, distribute,
reverse-associative, associative, identity) score 63-84% under the labeler bound and ~0% under
the strict one, because their job is enabling congruence closure rather than surviving into the
extracted expression, which the strict walk can't see by construction. This sharpens (doesn't
resolve) the "known follow-up" above: before training a Guide on labeler labels, the
union-causality tightening needs to be understood well enough to know whether it would reorder
the rule-priority ranking a Guide would learn — particularly for this structural-rule class,
where the two bounds disagree most. Full report: `docs/results/2026-08-30-guide-headroom.md` /
