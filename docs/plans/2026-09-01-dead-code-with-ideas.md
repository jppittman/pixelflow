...
| `pixelflow-pipeline/src/training/quarantine.rs` (deleted) (80–1308) | 1308 | LIVE — `grep -rl "training::quarantine" pixelflow-pipeline/src/bin` → gen_bench_corpus, bench_extraction_3way, bootstrap_extraction_head | one same-form JIT-vs-oracle gate shared by every label source, so a drifting duplicate predicate cannot reopen audit B8's leak | KEEP |
