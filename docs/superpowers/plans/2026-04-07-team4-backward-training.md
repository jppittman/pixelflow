...
> it will start building deleted types. `unified_backward.rs` and `train_unified.rs` were deleted with the
> self-play/REINFORCE loop in July 2026 — this plan's goal of deleting the
> first happened, by removing the whole program rather than by refactoring it.
