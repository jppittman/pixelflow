...
#[test]
fn verify_independent_instantiations() {
    let scale = kernel!(|factor: f32| X * factor);
...
#[test]
fn verify_zst_expression_is_copy() {
...
#[test]
fn verify_parameter_ordering_three() {
...
#[test]
fn verify_simple_kernel_composition() {
...
#[test]
fn verify_kernel_composition_with_offset() {
    let dist = kernel!(|cx: f32, cy: f32| {
        let dx = X - cx;
        let dy = Y - cy;
        (dx * dx + dy * dy).sqrt()
...
#[test]
fn verify_mixed_manifold_scalar_params() {
...
#[test]
fn verify_composed_kernel_ownership() {
    let dist = kernel!(|cx: f32, cy: f32| {
        let dx = X - cx;
        let dy = Y - cy;
        (dx * dx + dy * dy).sqrt()
...
#[test]
fn verify_jet3_multiple_literals() {
...
#[test]
fn verify_jet3_literals_and_params() {
    // offset + 2.0 * X - 0.5
...
#[test]
fn verify_fused_combinators_with_kernel_composition() {
...
#[test]
fn verify_manual_gradient_magnitude() {
...
//
// #[test]
// fn verify_derivative_accessors_with_composition() {
//     let circle_sdf = kernel!(|cx: f32, cy: f32, r: f32| -> Jet2 {
//         let dx = X - cx;
//         let dy = Y - cy;
//         (dx * dx + dy * dy).sqrt() - r
