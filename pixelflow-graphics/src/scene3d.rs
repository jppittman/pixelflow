...
pixelflow_compiler::kernel_raw!(pub struct Surface = |geometry: kernel, material: kernel, background: kernel| Jet3 -> Field {
...
pixelflow_compiler::kernel_raw!(pub struct ColorSurface = |geometry: kernel, material: kernel, background: kernel| Jet3 -> Discrete {
    // 1. Get distance t from geometry
    let t = geometry;
...
pixelflow_compiler::kernel_raw!(pub struct GeometryMask = |geometry: kernel| Jet3 -> Field {
    let t = geometry;
    let t_max = 1000000.0;
    let deriv_max = 10000.0;
...
pixelflow_compiler::kernel_raw!(pub struct Checker = || Jet3 -> Field {
...
pub fn sky() -> Lift<impl Manifold<Field4, Output = Field> + Clone> {
    Lift(pixelflow_compiler::kernel_raw!(|| {
