            ...
            if let Some(guard) = select_guards.iter().find(|g| g.select_idx == sched_idx) {
                ...
                if has_true || has_false {
                    ...
                    if let Some(freg) = false_reg {
                        ...
                        backend.emit_resolve(
                            &mut code, *false_vid, dst, &reg_for, &spill_for, &remat_for,
                        );
                    }
                    ...
                    if let Some(treg) = true_reg {
                        ...
                        backend.emit_resolve(
                            &mut code, *true_vid, dst, &reg_for, &spill_for, &remat_for,
                        );
                    }
...
mod tests {
    ...
    fn test_resolve_const() {
        ...
        let op = ScheduledOp::Const(std::f32::consts::PI);
        let plan = resolve_operands(&op, Loc::Reg(Reg(6)), &assign, &spills, &remat).unwrap();
        assert_eq!(
            plan.op,
            ResolvedOp::LoadConst {
                dst: Reg(6),
                val_bits: std::f32::consts::PI.to_bits()
            }
        );
    }
