...
/// tan(x) = sin(x) / cos(x).
///
/// Computes sin(x) into `dst`, then cos(x) into `scratch[3]`, then divides.
pub(crate) fn emit_tan_builtin(
    ...
    let s3 = scratch[3];
    // Keep sin(x) in dst so the cosine path can freely use scratch[3].
    emit_sin_body(code, pool, dst, src, scratch[0], scratch[1], scratch[2])?;
    // cos(x) → s3  (src is still intact — sin_body doesn't clobber it)
    emit_cos_builtin(code, pool, s3, src, scratch)?;
    // dst = sin(x) / cos(x)
    emit_fdiv(code, dst, dst, s3);
    Ok(())
}
