...
fn prod_swirl_kernel_through_nnue_and_jit() {
    ...
    for &(x, y) in &coords {
        ...
        if (got_orig - got_opt).abs() > 3e-1 {
            eprintln!("Warning: NNUE extraction changed semantics at ({},{}): original {:?} vs optimized {:?} (diff: {:?})", x, y, got_orig, got_opt, (got_orig - got_opt).abs());
        }
    }
    eprintln!(
