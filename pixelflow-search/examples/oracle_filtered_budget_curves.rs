...
//! `docs/results/2026-09-02-oracle-filtered-budget-curves.csv`).
...
use pixelflow_search::egraph::{
    ...
    collect_rule_templates, config_for_node_count, run_anytime_curve,
};
...
fn measure_expression(item: &CorpusItem) -> ExprMeasurement {
    ...
    // The curve's environment, named outright (#1108/#1118): this
    // expression's own production class cap, a generous sweep ceiling, and
    // no application cap of the optimizer's own — `run_anytime_curve`
    // supplies the application dimension one checkpoint gap at a time.
    // `hard_ceiling` is the wall clock's only remaining role: a panicking
    // safety ceiling, never a stop reason.
    let env = |rules: RuleSet| {
    ...
    // `RuleSet` owns its `Box<dyn Rewrite>`s and so is not `Clone`; each arm
    // gets its own. `RuleId`s are derived from rule identity, not from the
    // vector, so `full_rules`'s ids name the same rules in every copy.
    let full_rules = RuleSet::new(all_rules());
    let full_rules_count = full_rules.ids().len();
    // The journal is what the oracle rule set is DERIVED from, and after
    // #1118 recording is opt-in: `Optimizer::production()` observes nothing,
    // so an unobserved curve yields an empty `load_bearing` set and a
    ...
    // but recorded nothing means the observer was not attached, and the
    // oracle arm would then run with zero rules and look like a null result.
    assert!(
        best_out.egraph.application_count() == 0
            || best_out.egraph.provenance().recorded_count() > 0,
        "oracle: {} applications committed but the provenance journal is empty — \
         the curve was run without `observe(Some(KeepJournal))`",
        best_out.egraph.application_count()
    );
    ...
    // Keyed by `RuleId`, not by position: a rule vector's indices move under
    // any reordering or insertion, silently for a same-length reorder
    // (#1118). `RuleSet::id_of` is the only bridge between the two.
    let oracle_rule_ids: BTreeSet<RuleId> = labels
        .load_bearing
        .iter()
        .filter_map(|app_id| best_out.egraph.provenance().application(*app_id))
        .map(|record| {
            record.rule.unwrap_or_else(|| {
                panic!("oracle: application record carries no RuleId — the e-graph was built without rule ids")
            })
        })
        .collect();
    let oracle_rules = RuleSet::new(
        all_rules()
            .into_iter()
            .enumerate()
            .filter(|(idx, _)| {
                full_rules
                    .id_of(*idx)
                    .is_some_and(|id| oracle_rule_ids.contains(&id))
            })
            .map(|(_, r)| r)
            .collect::<Vec<_>>(),
    );
    ...
    let first_cost = uc.first().expect("non-empty grid").cost.dag;
    let final_cost = uc.last().expect("non-empty grid").cost.dag;
    ...
    let unguided_distinct_costs = {
        let set: BTreeSet<usize> = uc.iter().map(|c| c.cost.dag).collect();
        set.len()
    };
    ...
    ExprMeasurement {
        ...
        total_applications: best_out.egraph.provenance().recorded_count(),
        credited_non_direct,
    }
}
...
fn main() {
    ...
    let out_path = args
        ...
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            PathBuf::from("docs/results/2026-09-02-oracle-filtered-budget-curves.csv")
        });
